  ...
  <div class="mainbox">
    ...
    <div class="main-content-table">
      <el-breadcrumb separator-class="el-icon-arrow-right" class="BreadBox">
        ...
        <el-breadcrumb-item :to="{path:'/subofficeCon'}">{{areaName}}</el-breadcrumb-item>
...
export default {
  ...
  data() {
    ...
    if(this.areaName.length>0){
      this.areaName =  this.$route.query.name ; // 面包屑名称
      sessionStorage.setItem('twoPage',this.$route.query.name)

    }else{
      if (typeof  this.$route.query.name == "undefined"){
        this.areaName = sessionStorage.getItem('twoPage');
      }else{
        sessionStorage.setItem('twoPage',this.$route.query.name);
        this.areaName = sessionStorage.getItem('twoPage');
      }
